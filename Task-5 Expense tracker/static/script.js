...
async function initExpenses() {
  ...
  const render = expenses => { loaded = expenses; table.innerHTML = expenses.map(item => `<tr><td><b>${escapeHtml(item.title)}</b>${item.note ? `<br><small>${escapeHtml(item.note)}</small>` : ''}</td><td>${escapeHtml(item.category)}</td><td>${item.date}</td><td class="amount">${money(item.amount)}</td><td class="actions"><button class="small secondary" data-edit="${item.id}">Edit</button><button class="small danger" data-delete="${item.id}">Delete</button></td></tr>`).join('') || '<tr><td colspan="5" class="empty">No matching expenses.</td></tr>';
